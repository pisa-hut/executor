...
from executor.image_cache import resolve_sif
...
class ApptainerServiceConfig:
    ...
    def _resolve_sif_path(image_path: str) -> str:
        # All pull/cache/digest behavior lives in executor.image_cache; the
        # apptainer backend just asks it to turn an image_path into a local
        # .sif path. See that module for the digest-sidecar fast-path.
        return resolve_sif(image_path)
